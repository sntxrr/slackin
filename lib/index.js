    ...
    const captcha_callback = (err, resp) => {
      ...
      if (err) {
        ...
        if(resp.body.success){
          ...
          let hostname = email.split(/@/)[-1]

          if (blockDomains && blockDomains.includes(hostname)) {
          ...
          dns.resolveMx(hostname, (err, mxAddresses) => {
            ...
            if (blockDomains && mxAddresses && mxAddresses.some(mx => blockDomains.includes(mx.exchange))) {
